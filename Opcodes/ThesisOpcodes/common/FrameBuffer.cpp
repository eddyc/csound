...
template <typename T>
FrameBuffer<T>::FrameBuffer(size_t streamSize,
                            size_t frameSize,
                            size_t hopSize,
                            function<T*(size_t)> allocator)

    : inputBuffer(frameSize),
      ...
      hopSilence(allocator(hopSize), hopSize),
      streamSize(streamSize),
      frameSize(frameSize),
      hopSize(hopSize)
{
    if (streamSize != hopSize) {
        cout << "This doesn't work if streamsize is not equal to hopsize "
                "unfortunately!";
        cout << endl;
        exit(-1);
    }
                             ...
                             function<void(Vector<T>& inputFrame, Vector<T>& outputFrame)> callback)
{
    ...
    if (availableWrite == 0) {
        inputBuffer.peak((T*)(callbackInputFrame.data), callbackInputFrame.elementCount);
        ...
        outputBuffer.write(callbackOutputFrame.data, callbackOutputFrame.elementCount);
        outputBuffer.peak((T*)(olaFrame.data), olaFrame.elementCount);
