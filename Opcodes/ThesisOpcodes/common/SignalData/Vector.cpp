...
template <typename T> Vector<T>::Vector() : data(nullptr), elementCount(0) {}
...
Vector<T>::Vector(pair<T *, size_t> p)
    : data(p.first), elementCount(p.second) {}
...
template <typename T> Vector<T> &Vector<T>::map(function<T(T element)> func) {
    T *data = (T *)this->data;
...
template <typename T> T Vector<T>::reduce(function<T(T, T)> func) {
    T *data = (T *)this->data;
    T acc = 0;
    for (size_t i = 0; i < this->elementCount; ++i) {
        acc = func(acc, data[i]);
...
template <typename T> T Vector<T>::reduce(function<T(T, T, size_t)> func) {
    T *data = (T *)this->data;
    T acc = 0;
    for (size_t i = 0; i < this->elementCount; ++i) {
...
template <typename T> Vector<T> &Vector<T>::operator=(Vector<T> &&other) {
    if (this != &other) {
        ...
        memcpy((void *)&elementCount, (void *)&other.elementCount,
               sizeof(size_t));
    }
    return *this;
}

template <typename T> T &Vector<T>::last() {

    return *(T *)&this->data[elementCount - 1];
}

template <typename T> T *Vector<T>::begin() { return (T *)this->data; }
...
template <typename T> Vector<T> &Vector<T>::shift(int offset) {
    if (offset == 0)
        return *this;
    ...
    if (offset > 0) {
        ...
        memcpy((void *)data, (const void *)&data[abs(offset)],
               count * sizeof(T));
...
template <typename T>
StridedVector<T>::StridedVector(T *data, size_t capacity, size_t elementCount,
                                size_t stride)
    : data(data), capacity(capacity), elementCount(elementCount),
      stride(stride) {
    assert(((elementCount - 1) * stride) + 1 <= capacity);
}

template <typename T>
StridedVector<T> StridedVector<T>::subVector(size_t elementCount,
                                             size_t offset) {
    assert(elementCount + offset <= this->elementCount);
...
template <typename T> T &StridedVector<T>::operator[](size_t index) {
    assert(index < elementCount);
    T *data = (T *)this->data;
    return data[index * stride];
}

template <typename T> T &StridedVector<T>::operator[](size_t index) const {
    assert(index < elementCount);
    T *data = (T *)this->data;
    return data[index * stride];
}

template <typename T> void StridedVector<T>::print() {
    for (size_t i = 0; i < elementCount; ++i) {
...
template <typename T> void StridedVector<T>::printAll() {
...
template <typename T>
void StridedVector<T>::copy(const StridedVector<T> source,
...
ComplexVector<T>::ComplexVector(T *real, T *imag, size_t elementCount)
    : real(Vector<T>(real, elementCount)), imag(Vector<T>(imag, elementCount)),
