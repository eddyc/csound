...
template <typename T> void RingBuffer<T>::resize(size_t count) {
...
template <typename T> size_t RingBuffer<T>::getSize() const {
  return mAllocatedSize - 1;
}
//! Returns the number of elements available for wrtiing. \note Only safe to
//! call from the write thread.
template <typename T> size_t RingBuffer<T>::getAvailableWrite() const {
  return getAvailableWrite(mWriteIndex, mReadIndex);
}
//! Returns the number of elements available for wrtiing. \note Only safe to
//! call from the read thread.
template <typename T> size_t RingBuffer<T>::getAvailableRead() const {
...
//! TODO: consider renaming this to writeAll / readAll, and having generic read
//! / write that just does as much as it can
template <typename T> bool RingBuffer<T>::write(const T *array, size_t count) {
...
//! \return `true` if all elements were successfully read, or `false` otherwise.
//!
//! \note only safe to call from the read thread.
template <typename T> bool RingBuffer<T>::read(T *array, size_t count) {
...
template <typename T> bool RingBuffer<T>::peak(T *array, size_t count) {
  const size_t writeIndex = mWriteIndex.load(std::memory_order_acquire);
  const size_t readIndex = mReadIndex.load(std::memory_order_relaxed);
...
template <typename T> bool RingBuffer<T>::consume(size_t count) {
  const size_t writeIndex = mWriteIndex.load(std::memory_order_acquire);
  const size_t readIndex = mReadIndex.load(std::memory_order_relaxed);
...
template <typename T>
size_t RingBuffer<T>::getAvailableWrite(size_t writeIndex,
...
template <typename T>
size_t RingBuffer<T>::getAvailableRead(size_t writeIndex,
                                       size_t readIndex) const {
  if (writeIndex >= readIndex)
