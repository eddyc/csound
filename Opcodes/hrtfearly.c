...
static const int32_t elevationarray[14] = {56, 60, 72, 72, 72, 72, 72,
                                           60, 56, 45, 36, 24, 12, 1};
...
MYFLT filter(MYFLT *sig, MYFLT highcoeff, MYFLT lowcoeff, MYFLT *del,
  ...
  MYFLT freq;
  MYFLT check;
  MYFLT scale, nyqresponse, irttwo, highresponse, lowresponse, cosw, a, b, c, x,
      y;
...
typedef struct {
  ...
  MYFLT *idefroom, *ofade, *osr, *porder, *othreed, *Oheadrot, *ormx, *ormy,
      *ormz, *owlh, *owll, *owlg1, *owlg2, *owlg3, *oflh, *ofll, *oflg1, *oflg2,
...
static int32_t early_init(CSOUND *csound, early *p) {
  ...
  /* copy in string name...*/
  strNcpy(filel, (char *)p->ifilel->data,
          MAXNAME - 1); // filel[MAXNAME-1]='\0';
  strNcpy(filer, (char *)p->ifiler->data,
          MAXNAME - 1); // filer[MAXNAME-1]='\0';
  ...
  /* otherwise use values, if valid */
    ...
    p->wallcoefhigh =
        (*p->owlh > FL(0.0) && *p->owlh < FL(1.0)) ? *p->owlh : FL(0.3);
    p->wallcoeflow =
        (*p->owll > FL(0.0) && *p->owll < FL(1.0)) ? *p->owll : FL(0.1);
    p->wallg1 =
        (*p->owlg1 > FL(0.0) && *p->owlg1 < FL(10.0)) ? *p->owlg1 : FL(0.75);
    p->wallg2 =
        (*p->owlg2 > FL(0.0) && *p->owlg2 < FL(10.0)) ? *p->owlg2 : FL(0.95);
    p->wallg3 =
        (*p->owlg3 > FL(0.0) && *p->owlg3 < FL(10.0)) ? *p->owlg3 : FL(0.9);
    p->floorcoefhigh =
        (*p->oflh > FL(0.0) && *p->oflh < FL(1.0)) ? *p->oflh : FL(0.6);
    p->floorcoeflow =
        (*p->ofll > FL(0.0) && *p->ofll < FL(1.0)) ? *p->ofll : FL(0.1);
    p->floorg1 =
        (*p->oflg1 > FL(0.0) && *p->oflg1 < FL(10.0)) ? *p->oflg1 : FL(0.95);
    p->floorg2 =
        (*p->oflg2 > FL(0.0) && *p->oflg2 < FL(10.0)) ? *p->oflg2 : FL(0.6);
    p->floorg3 =
        (*p->oflg3 > FL(0.0) && *p->oflg3 < FL(10.0)) ? *p->oflg3 : FL(0.35);
    p->ceilingcoefhigh =
        (*p->oclh > FL(0.0) && *p->oclh < FL(1.0)) ? *p->oclh : FL(0.2);
    p->ceilingcoeflow =
        (*p->ocll > FL(0.0) && *p->ocll < FL(1.0)) ? *p->ocll : FL(0.1);
    p->ceilingg1 =
        (*p->oclg1 > FL(0.0) && *p->oclg1 < FL(10.0)) ? *p->oclg1 : FL(1.0);
    p->ceilingg2 =
        (*p->oclg2 > FL(0.0) && *p->oclg2 < FL(10.0)) ? *p->oclg2 : FL(1.0);
    p->ceilingg3 =
        (*p->oclg3 > FL(0.0) && *p->oclg3 < FL(10.0)) ? *p->oclg3 : FL(1.0);
  }
  ...
  if (!p->oldelevindex.auxp ||
      p->oldelevindex.size < impulses * sizeof(int32_t))
    csound->AuxAlloc(csound, impulses * sizeof(int32_t), &p->oldelevindex);

  if (!p->oldangleindex.auxp ||
      p->oldangleindex.size < impulses * sizeof(int32_t))
  ...
  if (!p->currentphasel.auxp ||
    ...
    csound->AuxAlloc(csound, irlength * impulses * sizeof(MYFLT),
                     &p->currentphasel);
  ...
  *p->irt60high = (FL(-0.161) * vol) / Salphahigh;
  ...
  if (threed)
    maxdist =
        HYPOT(maxdist, rmz); // maxdist = (SQRT(SQUARE(maxdist)+SQUARE(rmz)));
...
static int32_t early_process(CSOUND *csound, early *p) {
  ...
  int32_t elevindexlow, elevindexhigh, angleindex1, angleindex2, angleindex3,
      angleindex4;
  ...
  for (j = offset; j < nsmps; j++) {
    ...
    for (M = 0; M < impulses; M++) {
      ...
      outltot +=
          dell[pos] +
          frac * (dell[(pos + 1 < (maxdelsamps + skipdel[M]) ? pos + 1
                                                             : skipdel[M])] -
                  dell[pos]);
      outrtot +=
          delr[pos] +
          frac * (delr[(pos + 1 < (maxdelsamps + skipdel[M]) ? pos + 1
                                                             : skipdel[M])] -
                  delr[pos]);
      ...
      for (xc = -order; xc <= order; xc++) {
        for (yc = abs(xc) - order; yc <= order - abs(xc); yc++) {
          ...
          for (zc = lowz; zc <= highz; zc++) {
            ...
            if (srcx != p->srcxv || srcy != p->srcyv || srcz != p->srczv ||
                lstnrx != p->lstnrxv || lstnry != p->lstnryv ||
                lstnrz != p->lstnrzv || rotate != p->rotatev) {
              ...
              if (dist[M] < FL(0.45) && initialfade > irlength)
                ...
                bc = (SQRT(SQUARE(lstnrx - newpntx) + SQUARE(lstnry - newpnty) +
                ...
                if (bc == FL(0.0)) {
                  ...
                  /* cosine rule */
                  coselev = ((SQUARE(bc) + SQUARE(ab) - SQUARE(ac)) /
                             (2.0 * ab * bc));
                  /* VL: need to clamp it to avoid NaN */
                  elev =
                      (ACOS(coselev > 1.0 ? 1.0
                                          : (coselev < -1.0 ? -1.0 : coselev)) *
                       180.0 / PI_F);
                }
                ...
                angleindex = (int32_t)(
                    angle / (FL(360.0) / elevationarray[elevindex]) + FL(0.5));
                angleindex = angleindex % elevationarray[elevindex];

                /* avoid recalculation */
                angleindexlowstore =
                    angle / (FL(360.0) / elevationarray[elevindexlow]);
                angleindexhighstore =
                    angle / (FL(360.0) / elevationarray[elevindexhigh]);
                ...
                if (oldelevindex[M] != elevindex ||
                  ...
                  if (angleindex > elevationarray[elevindex] / 2) {
                    ...
                    for (i = 0; i < (elevationarray[elevindex] - angleindex);
                         i++)
                      skip += irlength;
                ...
                if (angleindex1 > elevationarray[elevindexlow] / 2) {
                  for (i = 0; i < elevindexlow; i++)
                    skip += ((int32_t)(elevationarray[i] / 2) + 1) * irlength;
                  for (i = 0; i < (elevationarray[elevindexlow] - angleindex1);
                       i++)
                    skip += irlength;
                  for (i = 0; i < irlength; i++) {
                ...
                if (angleindex2 > elevationarray[elevindexlow] / 2) {
                  for (i = 0; i < elevindexlow; i++)
                    skip += ((int32_t)(elevationarray[i] / 2) + 1) * irlength;
                  for (i = 0; i < (elevationarray[elevindexlow] - angleindex2);
                       i++)
                    skip += irlength;
                  for (i = 0; i < irlength; i++) {
                ...
                if (angleindex3 > elevationarray[elevindexhigh] / 2) {
                  for (i = 0; i < elevindexhigh; i++)
                    skip += ((int32_t)(elevationarray[i] / 2) + 1) * irlength;
                  for (i = 0; i < (elevationarray[elevindexhigh] - angleindex3);
                       i++)
                    skip += irlength;
                  for (i = 0; i < irlength; i++) {
                ...
                if (angleindex4 > elevationarray[elevindexhigh] / 2) {
                  for (i = 0; i < elevindexhigh; i++)
                    skip += ((int32_t)(elevationarray[i] / 2) + 1) * irlength;
                  for (i = 0; i < (elevationarray[elevindexhigh] - angleindex4);
                       i++)
                    skip += irlength;
                  for (i = 0; i < irlength; i++) {
                ...
                magllow = (FABS(lowl1[0])) +
                          ((FABS(lowl2[0]) - FABS(lowl1[0]))) * angleindex2per;
                maglhigh =
                    (FABS(highl1[0])) +
                    ((FABS(highl2[0]) - FABS(highl1[0]))) * angleindex4per;
                magrlow = (FABS(lowr1[0])) +
                          ((FABS(lowr2[0]) - FABS(lowr1[0]))) * angleindex2per;
                magrhigh =
                    (FABS(highr1[0])) +
                    ((FABS(highr2[0]) - FABS(highr1[0]))) * angleindex4per;
                ...
                magllow = (FABS(lowl1[1])) +
                          ((FABS(lowl2[1]) - FABS(lowl1[1]))) * angleindex2per;
                maglhigh =
                    (FABS(highl1[1])) +
                    ((FABS(highl2[1]) - FABS(highl1[1]))) * angleindex4per;
                magrlow = (FABS(lowr1[1])) +
                          ((FABS(lowr2[1]) - FABS(lowr1[1]))) * angleindex2per;
                magrhigh =
                    (FABS(highr1[1])) +
                    ((FABS(highr2[1]) - FABS(highr1[1]))) * angleindex4per;
                magl = magllow + (maglhigh - magllow) * elevindexhighper;
                magr = magrlow + (magrhigh - magrlow) * elevindexhighper;
                ...
                for (i = 2; i < irlength; i += 2) {
                  ...
                  maglhigh =
                      highl1[i] + (highl2[i] - highl1[i]) * angleindex4per;

                  magrlow = lowr1[i] + (lowr2[i] - lowr1[i]) * angleindex2per;
                  magrhigh =
                      highr1[i] + (highr2[i] - highr1[i]) * angleindex4per;
                ...
                /* all 4 walls are the same! (trivial to
                   make them different...) */
                /* x axis, wall1 (left) */
                wallreflections = (int32_t)abs(
                    (int32_t)(xc * .5 - .25 + (0.25 * pow(-1.0, xc))));
                /* wall2, x (right) */
                wallreflections += (int32_t)abs(
                    (int32_t)(xc * .5 + .25 - (0.25 * pow(-1.0, xc))));
                /* yaxis, wall3 (bottom) */
                wallreflections += (int32_t)abs(
                    (int32_t)(yc * .5 - .25 + (0.25 * pow(-1.0, yc))));
                /* yaxis, wall 4 (top) */
                wallreflections += (int32_t)abs(
                    (int32_t)(yc * .5 + .25 - (0.25 * pow(-1.0, yc))));
                if (threed) {
                  /* floor (negative z) */
                  floorreflections = (int32_t)abs(
                      (int32_t)(zc * .5 - .25 + (0.25 * pow(-1.0, zc))));
                  /* ceiling (positive z) */
                  ceilingreflections = (int32_t)abs(
                      (int32_t)(zc * .5 + .25 - (0.25 * pow(-1.0, zc))));
                }
                ...
                if (threed) {
                  for (i = 0; i < floorreflections; i++) {
                    delsinglel = delsingler = FL(0.0);
                    filter(hrtflinterp, p->floorcoefhigh, p->floorcoeflow,
                           &delsinglel, irlength, sr);
            ...
            csound->RealFFTMult(csound, outlspec, hrtflpad, inbufpad,
                                irlengthpad, FL(1.0));
            csound->RealFFTMult(csound, outrspec, hrtfrpad, inbufpad,
                                irlengthpad, FL(1.0));
            ...
            if (crossout)
              for (i = 0; i < irlength; i++) {
                predell[i + M * irlength] =
                    ...
                    (outlspec[i] + (i < overlapsize ? overlapl[i] : FL(0.0))) *
                        (FL(l[M]) / fadebuffer);
                predelr[i + M * irlength] =
                    (outrspecold[i] +
                     (i < overlapsize ? overlaprold[i] : FL(0.0))) *
                        FL(1. - (FL(l[M]) / fadebuffer)) +
                    (outrspec[i] + (i < overlapsize ? overlapr[i] : FL(0.0))) *
                        (FL(l[M]) / fadebuffer);
                l[M]++;
              }
            else
...
static OENTRY hrtfearly_localops[] = {{"hrtfearly", sizeof(early), 0, 3,
                                       "aaiii",
